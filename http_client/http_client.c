...
int send_http_to_server(char *pRequestUrl, char *pSendBody, void *pAlg_ability_request_callback)
{
    if (pRequestUrl == NULL)
    {
        fprintf(stderr, "the param is NULL!\n");
        return -1;
    }
    struct evhttp_uri* uri = evhttp_uri_parse(pRequestUrl);
    if (!uri)
    {
    ...
    if (pAlg_ability_request_callback != NULL)
    {
        evhttp_request_set_chunked_cb(request, pAlg_ability_request_callback);
    }
    // else
    // {
    ...
    const char* request_url = pRequestUrl;
    ...
    char szHost[64] = {0};
    sprintf(szHost, "%s:%d", host, port);

    evhttp_add_header(evhttp_request_get_output_headers(request), "Host", szHost);
    evhttp_add_header(evhttp_request_get_output_headers(request), "Content-type", "application/json");

    struct evbuffer* evbuf = evbuffer_new();
    evbuf = evhttp_request_get_output_buffer(request);

    
    if (pSendBody != NULL)
    {
        evbuffer_add(evbuf, pSendBody ,strlen(pSendBody));
    }
    

    evhttp_make_request(connection, request, EVHTTP_REQ_POST, path);
